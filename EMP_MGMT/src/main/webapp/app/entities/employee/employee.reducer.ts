...
export const ACTION_TYPES = {
  ...
  FETCH_EMPLOYEE_LIST_BY_TEAM: 'employee/FETCH_EMPLOYEE_LIST_BY_TEAM',
...
export default (state: EmployeeState = initialState, action): EmployeeState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.FETCH_EMPLOYEE_LIST_BY_TEAM):
    ...
    case FAILURE(ACTION_TYPES.FETCH_EMPLOYEE_LIST_BY_TEAM):
...
const apiTeamUrl = 'api/employee/team';
...
export const getEntitiesByTeam: ICrudGetAction<IEmployee> = id => {
  const requestUrl = `${apiTeamUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_EMPLOYEE_LIST_BY_TEAM,
